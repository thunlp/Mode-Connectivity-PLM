...
class Trainer:
    ...
    def get_optimzied_group(self):
        if self.args.tune_method == 'model':
            ...
            optimizer_grouped_parameters = []
            for n, p in self.model.named_parameters():                
                if not (self.args.apply_adapter or self.args.apply_lora or self.args.apply_prefix):
                    #全参数的curve find
                    p.requires_grad = True
                    optimizer_grouped_parameters.append({'params': [p]})
                    print(n)
                    ...
                    p.requires_grad = False
                    if 'train_theta' in n:
                    # if self.args.pet_type in n or 'train_theta' in n:
                        p.requires_grad = True
                        optimizer_grouped_parameters.append({'params': [p]})
                        print(n)
            to_update = self.model.parameters()
        ...
        for t in valid_points:
            ...
            with torch.no_grad():
                for local_step, batch in enumerate(valid_dataloader):
                    ...
                    valid_loss = self.model(all_input)                
                    self.add_logging(valid_log_dict, 'loss', valid_loss.item())
                    
                    decoder_input_ids = self.get_decoder_input_ids(all_input["inputs_embeds"])
                    
                    n = self.model.bend_num
                    P_alpha = pow(1-t, n+1) * self.model.end_w[0] + pow(t, n+1) * self.model.end_w[1]
                    coeff = []
                    for i in range(n):
                        coeff.append(pow(t, i+1) * pow(1-t, n-i) * comb(n+1,i+1))
                    flatten_pet = P_alpha + torch.Tensor(coeff).cuda() @ self.model.train_theta
    ...
    def test(self, epoch=0, num_updates=0,t=0.5, test_data=None):
        ...
        with torch.no_grad():
            for local_step, batch in enumerate(test_dataloader):
                ...
                test_loss = self.model(all_input)                
                ...
                if self.args.apply_adapter:
                    ...
                    gen_text = self.generate_text(all_input,decoder_input_ids,only_adapter=False,only_lora=False,only_prefix=True)
                    my_prediction_finetune.extend(gen_text)
        if len(my_prediction_adapter) != 0:
            # metrics = self.evaluate(my_prediction, my_index, self.valid_dataset)
