...
from modeling_t5_layer_weight_find_finetune import layer_weight_find_finetune
...
class Trainer:
    def __init__(self, args, logger, model_provider):
        ...
        self.model, self.ID_wrap = layer_weight_find_finetune(self.model, args, device="cuda")
        ...
        if self.args.tune_method == 'model':
            ...
            for n, p in self.model.named_parameters():                
                ...
                if 'train_theta' in n:
                    ...
                    print(n)
            to_update = self.model.parameters()
        elif self.args.tune_method == 'weight_find':
            no_decay = ["bias", "layer_norm.weight"]
            optimizer_grouped_parameters = []
            for n, p in self.model.named_parameters():                
                p.requires_grad = False
                if 'layer_weight' in n:
                    p.requires_grad = True
                    optimizer_grouped_parameters.append({'params': [p]})
                    print(n)
            to_update = self.model.parameters()
    ...
    def valid(self, epoch=0, num_updates=0):
        ...
        with torch.no_grad():                   
            ...
            for local_step, batch in enumerate(valid_dataloader):
                ...
                valid_loss = self.model(**all_input)['loss']                
        ...
        if len(my_prediction_adapter) != 0:
            ...
            metrics[metric_key] = list(metrics_finetune.values())[0]
    ...
    def test(self, epoch=0, num_updates=0, test_data=None):
        ...
        with torch.no_grad():
            ...
            for local_step, batch in enumerate(test_dataloader):
                all_input = self.prepare_data(batch)
                decoder_input_ids = self.get_decoder_input_ids(all_input["input_ids"])
                test_loss = self.model(**all_input)['loss']                
    ...
    def get_pet_type(self, args):
        assert args.tune_method == 'curve_find' or args.tune_method == 'weight_find', "Function get_pet_type must be used in curve find or weight find!"
        # assert args.apply_lora + args.apply_adapter + args.apply_prefix == 1, "not only one method applied in curve find! "
        if args.apply_lora:
            pet_type = 'lora'
    ...
    def save_checkpoint(self, path, epoch, num_updates):
        ...
        if self.args.tune_method == 'model':
            ...
            # save lora
            my_state_dict = self.model.state_dict()
            state_dict['weight_find'] = {k: my_state_dict[k] for k in my_state_dict if 'layer_weight' in k}
    ...
    def load_checkpoint(self, path):
        ...
        if state_dict['args']['tune_method'] == 'model':
            ...
            self.model.load_state_dict(model_dict)
        elif state_dict['args']['tune_method'] == 'weight_find':
            model_dict = {k: v for (k, v) in self.model.state_dict().items()}
            model_dict.update({k: v.cuda() for (k, v) in state_dict['weight_find'].items()})
            self.model.load_state_dict(model_dict)
    ...
    def train_step(self, batch):
        all_input = self.prepare_data(batch)
        loss = self.model(**all_input)['loss']
