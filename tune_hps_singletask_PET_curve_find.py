...
from T5_model.modeling_t5_PET_curve_find import (
    # T5Model,
    MyT5_pet_CF,
    # T5Tokenizer,
    # T5Config
...
from T5_model.t5_trainer_PET_CF import Trainer
...
def model_provider(args):
    ...
    model = MyT5_pet_CF(args, config=config)
        ...
        for bsz in args.bsz_list:
            for lr in args.learning_rate_list:
                ...
                if args.do_predict:
                    ...
                    dev_data = trainer.dev_data
                    test_data = trainer.test_data
                    for x in np.linspace(0, 1, args.itpl_points):
                        logger.info("Test ... prefix={}, x={}...".format(prefix, x))
                        metric, dev_performance, dev_loss = trainer.test(t=x, test_data=dev_data)
                        metric, test_performance, test_loss = trainer.test(t=x, test_data=test_data)
                        if dev_performance > best_dev_performance:
                            best_dev_performance = dev_performance
                            max_df_line = [prefix, metric+'_best', x, dev_performance, dev_loss, test_performance, test_loss]

                        result_name = "result.csv"
                        df.loc[len(df.index)] = [prefix, metric, x, dev_performance, dev_loss, test_performance, test_loss]
                        df.to_csv(os.path.join(output_dir, result_name),sep=',',index=False,header=True, float_format='%.4f')
                
                df.loc[len(df.index)] = max_df_line
                df.to_csv(os.path.join(output_dir, result_name),sep=',',index=False,header=True, float_format='%.4f')
        if args.one_prefix:
            break
