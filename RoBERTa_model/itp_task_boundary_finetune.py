...
class ModelArguments:
    ...
    path1: str = field(
        metadata={"help": "lhs checkpoint to be interpolated"}
    )
    path2: str = field(
        metadata={"help": "rhs checkpoint to be interpolated"}
    )
...
def write_result(output_dir, result_name, dev_performance, best_dev_performance, valid_loss, test_performance, test_loss, df, prefix, metric, x):
    best_config = None
    ...
    if dev_performance > best_dev_performance:
        ...
        best_config = [prefix, metric, x, best_dev_performance, best_valid_loss, best_test_performance, best_test_loss]
...
def main():
    ...
    # mnli has three labels, need to change when task changes
    ...
    # sst2 has two labels, need to change when task changes
    ...
    for x in np.linspace(0, 1, 26):
        model_dict_to_update = {key: ((1-x)*model1.state_dict()[key].cuda() + x*model2.state_dict()[key].cuda())
                            for key in list(model1.state_dict().keys())[:-2]}
        model_dict.update(model_dict_to_update)

        # the classifier needs to be given value independently
        if data_args.task_name == "mnli":
            classification_dict = {"classifier.out_proj.weight": model1.state_dict()["classifier.out_proj.weight"], \
                "classifier.out_proj.bias": model1.state_dict()["classifier.out_proj.bias"]}
        elif data_args.task_name == "sst2" or data_args.task_name == "qqp":
            classification_dict = {"classifier.out_proj.weight": model2.state_dict()["classifier.out_proj.weight"], \
                "classifier.out_proj.bias": model2.state_dict()["classifier.out_proj.bias"]}
        ...
        for eval_dataset, task in zip(eval_datasets, tasks):
            ...
            if task == "mnli-mm":
                ...
                best_config, best_mm_performance, df = write_result(training_args.output_dir, result_name, 0, best_dev_performance, 0, metrics["eval_accuracy"], metrics["eval_loss"], df, task, "acc", x)
