...
def main():
    ...
    for x in np.linspace(0, 1, 26):
        ...
        delta_model = AutoDeltaModel.from_finetuned(model_args.path1, backbone_model=model)
        delta_model1 = AutoDeltaModel.from_finetuned(model_args.path1, backbone_model=model1)
        delta_model2 = AutoDeltaModel.from_finetuned(model_args.path2, backbone_model=model2)
        ...
        model_dict_to_update = {key: ((1-x)*model1.state_dict()[key].cuda() + x*model2.state_dict()[key].cuda())
                            for key in list(model1.state_dict().keys())}
